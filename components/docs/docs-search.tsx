...
export function DocsSearch({ query, onQueryChange, results, onSelect }: Props) {
  return (
    <div className="border-b border-[rgba(255,255,255,0.08)] p-3">
      <input
        className="w-full rounded border border-[rgba(255,255,255,0.12)] bg-[var(--flux-surface-elevated)] px-3 py-2 text-sm text-[var(--flux-text)] outline-none focus:border-[var(--flux-primary)]"
        placeholder="Buscar docs..."
        value={query}
        onChange={(e) => onQueryChange(e.target.value)}
      />
      {query.trim() && (
        <div className="mt-2 max-h-[180px] overflow-auto rounded border border-[rgba(255,255,255,0.08)] bg-[var(--flux-surface-mid)]">
          {results.length === 0 ? (
