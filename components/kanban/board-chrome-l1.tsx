...
const MOBILE_VIEW_LABEL_KEYS: Record<BoardViewMode, string> = {
  ...
  return (
    <>
      ...
      {nlqExpanded && !onda4Omnibar ? (
        ...
        <>
        <div className="md:hidden flex min-w-0 items-center gap-2 border-b border-[var(--flux-chrome-alpha-08)] flux-glass-surface rounded-none border-x-0 border-t-0 px-3 py-2">
          <label className="min-w-0 flex-1">
            <span className="sr-only">{t("board.mobileViewPicker.label")}</span>
            <select
              value={boardView}
              onChange={(e) => setBoardView(e.target.value as BoardViewMode)}
              className="h-10 w-full min-w-0 rounded-xl border border-[var(--flux-chrome-alpha-14)] bg-[var(--flux-surface-elevated)] px-3 text-xs font-semibold text-[var(--flux-text)] outline-none focus:border-[var(--flux-primary)] focus:ring-2 focus:ring-[var(--flux-primary-alpha-22)]"
              aria-label={t("board.mobileViewPicker.label")}
            >
              {allowedViewModes.map((mode) => (
                <option key={mode} value={mode}>
                  {tTimeline(MOBILE_VIEW_LABEL_KEYS[mode])}
                </option>
              ))}
            </select>
          </label>
          ...
          {onOpenFilterModal ? (
            <button
              ...
              className="inline-flex h-10 min-w-[44px] shrink-0 items-center justify-center rounded-xl border border-[var(--flux-chrome-alpha-14)] bg-[var(--flux-surface-elevated)] text-[var(--flux-text-muted)] shadow-sm transition-colors hover:border-[var(--flux-primary-alpha-35)] hover:text-[var(--flux-primary-light)] hover:bg-[var(--flux-primary-alpha-08)]"
            >
          ...
          {!onda4Omnibar ? (
            <button
              ...
              className="inline-flex h-10 min-w-[44px] shrink-0 items-center justify-center rounded-xl border border-[var(--flux-chrome-alpha-14)] bg-[var(--flux-surface-elevated)] text-[var(--flux-text-muted)] transition-colors hover:text-[var(--flux-text)] hover:border-[var(--flux-primary-alpha-35)]"
        ...
        <div className="hidden md:flex min-w-0 flex-nowrap items-center gap-2 overflow-x-auto overflow-y-hidden border-b border-[var(--flux-chrome-alpha-08)] flux-glass-surface rounded-none border-x-0 border-t-0 px-4 py-1.5 scrollbar-flux touch-pan-x sm:px-5 lg:px-6">
