...
export function boardChromeStickyRootClass(variant: BoardChromeSurfaceVariant): string {
  ...
  const sticky =
    "sticky top-[calc(env(safe-area-inset-top,0px)+3.75rem)] md:top-[42px] z-[var(--flux-z-board-sticky-chrome)] flex min-w-0 max-w-full flex-col overflow-x-clip rounded-none border-x-0 border-t-0";
