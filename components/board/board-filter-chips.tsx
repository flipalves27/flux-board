...
/** Priority + label filter chips for board headers and toolbars. */
export function BoardFilterChips({ boardId, hidePriorities = false }: { boardId: string; hidePriorities?: boolean }) {
  const activePrio = useFilterStore((s) => s.filtersByBoard[boardId]?.activePrio ?? "all");
