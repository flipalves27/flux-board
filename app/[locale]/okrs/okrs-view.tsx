...
import {
  ...
  OkrsRouteLoadingFallback,
} from "@/components/skeletons/route-loading-fallbacks";
import {
  computeOkrsProgress,
...
export default function OkrsView() {
  ...
  const [okrFetchBusy, setOkrFetchBusy] = useState(false);

  const authWaiting = !isChecked || !user;
  const showDataSkeleton = useMinimumSkeletonDuration(
    (!boardsLoaded || okrFetchBusy) && !authWaiting
  );
  ...
  if (authWaiting) {
    return <OkrsRouteLoadingFallback />;
  }
