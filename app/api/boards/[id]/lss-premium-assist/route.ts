...
import { LssPremiumAssistBodySchema, sanitizeText, zodErrorToMessage } from "@/lib/schemas";
import { guardUserPromptForLlm } from "@/lib/prompt-guard";
import { runOrgLlmChat } from "@/lib/llm-org-chat";
import {
  buildLssPremiumAssistSystemPrompt,
  buildLssPremiumAssistUserPrompt,
  type LssPremiumAssistMode,
} from "@/lib/lss-premium-assist-prompt";

export const runtime = "nodejs";

/**
 * IA C-level / premium para boards Lean Six Sigma. Plano Business+ (`lss_ai_premium`).
 */
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (board.boardMethodology !== "lean_six_sigma") {
    return NextResponse.json(
      { error: "Assistente premium LSS disponível apenas em boards Lean Six Sigma." },
      { status: 400 }
    );
  }
  ...
  const rl = await rateLimit({
    key: `boards:lss-premium-assist:user:${payload.id}`,
    limit: 12,
    windowMs: 60 * 60 * 1000,
  });
  if (!rl.allowed) {
  ...
  const parsed = LssPremiumAssistBodySchema.safeParse(body);
  if (!parsed.success) {
    return NextResponse.json({ error: zodErrorToMessage(parsed.error) }, { status: 400 });
  }

  const mode = parsed.data.mode as LssPremiumAssistMode;
  ...
  const system = buildLssPremiumAssistSystemPrompt(mode);
  const userContent = buildLssPremiumAssistUserPrompt(context, cardSnippet || undefined);

  const res = await runOrgLlmChat({
    org,
    orgId: payload.orgId,
    feature: "board_lss_premium_assist",
    mode: "interactive",
    userId: payload.id,
    ...
    options: { maxTokens: 3200, temperature: 0.35 },
  });
