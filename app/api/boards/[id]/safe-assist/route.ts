...
import { SafeAssistBodySchema, sanitizeText, zodErrorToMessage } from "@/lib/schemas";
import { guardUserPromptForLlm } from "@/lib/prompt-guard";
import { runOrgLlmChat } from "@/lib/llm-org-chat";
import {
  buildSafeAssistSystemPrompt,
  buildSafeAssistUserPrompt,
  type SafeAssistMode,
} from "@/lib/safe-assist-prompt";
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    assertFeatureAllowed(org, "safe_ai_premium", gateCtx);
  } catch (err) {
  ...
  if (board.boardMethodology !== "safe") {
    return NextResponse.json(
      { error: "Assistente SAFe disponível apenas em boards com metodologia SAFe (fluxo aproximado)." },
      { status: 400 }
    );
  }
  ...
  const rl = await rateLimit({
    key: `boards:safe-assist:user:${payload.id}`,
    limit: 12,
    windowMs: 60 * 60 * 1000,
  });
  if (!rl.allowed) {
  ...
  const parsed = SafeAssistBodySchema.safeParse(body);
  if (!parsed.success) {
    return NextResponse.json({ error: zodErrorToMessage(parsed.error) }, { status: 400 });
  }

  const mode = parsed.data.mode as SafeAssistMode;
  ...
  const system = buildSafeAssistSystemPrompt(mode);
  const userContent = buildSafeAssistUserPrompt(context, cardSnippet || undefined);

  const res = await runOrgLlmChat({
    org,
    orgId: payload.orgId,
    feature: "board_safe_assist",
    mode: "interactive",
    userId: payload.id,
