...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const rl = await rateLimit({
    key: `account-brief-ai:${payload.orgId}:${boardId}`,
    limit: 12,
    windowMs: 60 * 60_000,
  });
  if (!rl.allowed) {
  ...
  const cards = (board.cards ?? []) as Array<{
    bucket?: string;
    priority?: string;
    progress?: string;
    title?: string;
  }>;
  const cardSummaryLines = cards.map(
    (c) =>
      `- ${String(c.bucket ?? "")} · ${String(c.priority ?? "")} · ${String(c.progress ?? "")} · ${String(c.title ?? "").slice(0, 120)}`
  );
  ...
  const userPrompt = buildAccountBriefUserPrompt({
  ...
  const res = await runOrgLlmChat({
    org,
    orgId: payload.orgId,
    feature: "board_account_brief_ai",
    mode: "batch",
    messages: [{ role: "user", content: userPrompt }],
    options: { maxTokens: 1000, temperature: 0.35 },
  });

  if (!res.ok) {
    return NextResponse.json(
      { error: res.error ?? "Falha ao gerar brief" },
