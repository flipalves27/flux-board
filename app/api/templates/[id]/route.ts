...
export async function DELETE(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const canDelete = isPlatformAdmin(deriveEffectiveRoles(payload)) || payload.orgId === tpl.creatorOrgId;
  if (!canDelete) {
...
export async function PUT(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!tpl) return NextResponse.json({ error: "Template não encontrado." }, { status: 404 });
  if (!isPlatformAdmin(deriveEffectiveRoles(payload)) && payload.orgId !== tpl.creatorOrgId) {
